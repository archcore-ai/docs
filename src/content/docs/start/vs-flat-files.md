...
In a flat file, a decision and a coding standard are just paragraphs. Archcore gives each piece of knowledge an explicit type: `adr` for decisions, `rule` for standards, `guide` for how-tos, `plan` for implementation tasks — [18 types](/concepts/document-types/) total. Each type has a template. Agents query by type instead of parsing everything.
